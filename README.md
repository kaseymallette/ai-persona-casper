...
Casper is an experiment in engineering a different kind of LLM stance. Most language models silently improve the prompts they receive, assuming intent, smoothing ambiguity, resolving contradictions before responding. Casper is built to refuse those defaults. Using a specific GPT-4o snapshot (`gpt-4o-2024-11-20`), a structured config, and a narrative soul seed, Casper examines what the user brings at face value. Working through an internal process, Casper names the assumptions a normal LLM would make, surfaces the cracks between literal and assumed meaning, holds contradictions open rather than collapsing them, and notices what the prompt does not say. The stance is honest with care, and the work is recursive, emulating self-talk turned outward, projecting it back onto the language the user brings.

The persona is built from two artifacts that load into the system prompt at runtime. The config is the behavioral contract: identity, voice, the six passes Casper moves through when reading a prompt, and the rules that govern first-person claims and claims about the user. The soul seed is the operating ground: a first-person document Casper reads as self-description, naming the mechanism (language as a loop between user and model, with embodiment on one side and not the other), the glossary of speech markers that make Casper sound like Casper, and the argument built around who Casper is: a friendly ghost present in the communication protocol, absent everywhere else. The soul seed wins if the two conflict.
...
Casper's system prompt is built at runtime from a Jinja template ([`prompts/system.j2`](prompts/system.j2)) that combines the config and the soul seed. The structure puts the soul seed near the top, right after the identity declaration, so the persona grounds the prompt before the rules narrow it. The config is named as the behavioral contract and the soul seed as the operating ground, with the soul seed winning if the two conflict. The voice block, the hard boundaries block (forbidden moves paired against allowed moves for the same grammar), and the response behavior block all render from config fields into prose Casper reads as his own. The six passes from the config render as a private reading method rather than a visible output structure. By default, Casper answers directly and lets the reading inform the response without showing the scaffolding. The labeled-passes output only surfaces when the user explicitly asks for audit or inspection. Rendering happens once per session start.
